...
public class Bot extends TelegramLongPollingBot {
    ...
    public void onUpdateReceived(Update update) {
        if (update.hasMessage() && update.getMessage().hasText()) {
            ...
            if(message_text.equalsIgnoreCase(helpCommnd) )
            {
                String helpAnswer = "Available commands: \n" +
                        "/start - start sending ads \n" +
                        "/stop  - stop sending ads \n" +
                        "/help  - view list of commands. \n" +
                        "To begin get latest ads from me  - " +
                        "please send me a search link from olx ua fallowing after /start command.";
            ...
            else
            {
                SendMessage message = new SendMessage()
                        .setChatId(chat_id)
                        .setText("Sorry, you doing something wrong . Try to use /help command to know more .");
                try {
                    execute(message);
                } catch (TelegramApiException e) {
                    e.printStackTrace();
